...
1. The parent Spec (`teps/TEP-{t}/SP-{n}/spec.md`) — specifically the acceptance criteria + the design + the file plan.
2. The current diff (`git diff main...HEAD` or `gh pr diff` if a PR exists).
3. The slice file (`teps/TEP-{t}/SP-{n}/SL-{m}.md`) for context on what specifically this card delivers.
