...
- **Role.** The default `role: "code"` authors the implementation. Additionally, one **`role: "test"` test-author unit** writes the slice's **held-out acceptance probe** — footprint `acceptance/SP-{n}_SL-{m}.*`, `consumes`-ing the slice's contract file, never touching the code-authors' files. Because its footprint sits under `acceptance/` (SP-7's held-out fence keeps a code-author out), it runs **in parallel with, and independent of, the code-authors** — the grade is authored by a hand that never wrote the implementation. Emit exactly one per implementation slice; `role` is orthogonal to `execution` and `footprint`.
...
- `$ARGUMENTS`: the Spec id `{n}` — a sequential integer minted per thinking space+org (`SP-1`, `SP-2`, …). The spec and its slices nest under the parent TEP `{t}` (from the spec's `implements:`).
...
3. **Brainstorm slices privately.** Working through the Design + File Structure Plan, draft candidate slices — cut **vertically** (coherent end-to-end behaviours), not by layer/file. For each, check:
   ...
   - Does it **consume an artifact another unit produces**? Name that artifact with `consumes` — the orchestrator resolves it globally against footprints to build the unit DAG, the only scheduling graph. Remember **a later slice is not a dependency** — if you can't name the artifact, there is no dependency. Can it run independently of its siblings (no shared file/state edits, no required ordering)? If so, mark `parallel: true` — _parallel-eligible_, not must-run-in-parallel.
     ...
     3b. **Emit the held-out test-author unit — the role axis (who _grades_ the slice).** After the code work units are clustered, add **one `role: "test"` unit** whose `footprint` is `acceptance/SP-{n}_SL-{m}.*` (under SP-7's held-out `acceptance/` fence) and whose `consumes` names the slice's **contract file** — the shared type/name/schema/message the code-authors also consume. It carries no code footprint, so SP-7's fence keeps a code-author out of it and it runs **in parallel with, and independent of, the `role: "code"` units** — the grade is authored by a hand that never wrote the implementation. Default `role` for every other unit is `"code"` (omit it); emit exactly one test-author unit per implementation slice. See `reference.md` ("The role axis") for the rationale and the worked example.
