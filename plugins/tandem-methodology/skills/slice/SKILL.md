...
- **Contract — the shared seam, established at design, SHARED ACROSS THE SPEC.** Each multi-unit slice declares a **`contract`**: the exact interface (exports, types, signatures, behaviour) it introduces — **~10–20 lines you write WHEN THE SLICE IS CREATED**, not a work unit, not derived, not prose. The orchestrator injects the **union of every slice's contract** — the whole feature's seam — into **every** worker's prompt (code and held-out test, in **any** slice), so a unit builds against an interface **another slice defines** (a webview slice against the token/store a headless-gate slice owns). This is the **cross-slice interface agreement** that `consumes`-between-slices used to carry — now the contract carries it. So `consumes` shrinks to its honest core: a **genuine produced-artifact dependency** (a unit that ingests another unit's OUTPUT — a generated file, a schema, a migration seed), which is **still resolved Spec-globally / cross-slice**. A well-contracted feature like token+store+gate+webview has **zero `consumes`** — every unit builds to the shared contract, fully parallel across all slices. `create_slice` **requires** a contract for any multi-unit slice (there is no no-contract path).
- **Role — who _grades_ the slice.** The default `role: "code"` authors the implementation. Alongside the code units, add **held-out `role: "test"` unit(s)** that write the slice's **acceptance probes** — **one probe per AC the slice `satisfies`**, at the path the **repo declares** in `.tandem/conventions.json` → `acceptanceProbe.sourcePath` (e.g. this repo's `src/acceptance/SP-{spec}_AC-{k}.test.ts`; a Python repo declares its own `pytest` path). That footprint sits under `acceptance/` (the held-out fence keeps a code-author out) and **is exactly the `ac_verifications` command the auditor declared** for each AC. The test-author builds its probe **against the CONTRACT + the ACs** — so it `consumes` **nothing** (its independence comes from being a separate, spec-driven session behind the fence, not from a dependency) and runs **fully parallel with the code-authors**, blind to their implementation. Decompose the probes by the **same footprint rule as code**: distinct ACs → one held-out unit **per AC** (fan-out); ACs that share a heavy harness → cluster into one. (No `.tandem/conventions.json` recipe ⇒ no held-out probe path exists; the AC falls back to the repo's whole-suite command and grades self — declaring the recipe turns independence on.)
...
3. **Brainstorm slices privately.** Working through the Design + File Structure Plan, draft candidate slices — cut **vertically** (coherent end-to-end behaviours), not by layer/file. For each, check:
   ...
   - Does a unit **ingest another unit's produced OUTPUT** (a generated file, schema, migration seed — real data, not an interface)? Only THAT is a `consumes`. Name the artifact and the orchestrator resolves it **globally across the spec's units** (every slice's work_units) to build the unit DAG — a cross-slice edge is normal (the slice is only a validation envelope, never a scheduling boundary). **Interface agreement is NOT a `consumes`** — that's what the slice `contract` is for (units build to the contract, not to each other), so most contract-defined slices have **zero `consumes`**. Remember **a later slice is not a dependency** — if you can't name a produced artifact, there is no dependency. Can it run independently of its siblings? If so, mark `parallel: true` — _parallel-eligible_, not must-run-in-parallel.
     ...
     3b. **Write the slice's `contract` (design-time seam, shared Spec-wide).** For any multi-unit slice, write the shared interface it introduces — the exact exports, types, signatures and behaviour (~10–20 lines). This is the `contract` you pass to `create_slice`. The orchestrator injects the **union of every slice's contract** into every worker across **all** slices, so a later slice's units build against the interfaces this slice defines (cross-slice agreement) **without** a `consumes`. `create_slice` **refuses a multi-unit slice with no contract**.

     3c. **Emit the held-out test-author unit(s) — the role axis (who _grades_ the slice).** Alongside the code units, add **`role: "test"` unit(s)** whose `footprint` is the **repo's held-out acceptance-probe path** (`.tandem/conventions.json` → `acceptanceProbe.sourcePath`, filled with the spec id and **each AC ordinal the slice `satisfies`** — this repo's `src/acceptance/SP-{spec}_AC-{k}.test.ts`). Each probe path is exactly the `ac_verifications` command the auditor declared for that AC. The test-author builds against the **`contract` + the ACs** (both in its prompt) — so it `consumes` **nothing**; its independence is the separate spec-driven session behind the `acceptance/` fence, not a dependency. It runs **fully parallel with, and blind to, the `role: "code"` units**. Decompose by the **same footprint rule as code**: distinct ACs → **one held-out unit per AC** (fan-out); ACs sharing a heavy harness → cluster. Default `role` for every other unit is `"code"` (omit it). See `reference.md` ("The role axis") for the rationale and the worked example.
       - **Write each test unit's `note` as a PLAIN statement of the behaviour to assert** — it is injected verbatim into the worker's task. State only *what must be true* (e.g. `note: "create_slice and the spec→Ready transition are refused when the approval store holds no valid approval"`). **Never** put grading-mechanism vocabulary in it — no "held-out", "probe", "grader", "independent verifier", "ac_verifications", "consume nothing", "the evidence the gate grades". Those tell the worker it is being independently graded, which invites gaming; the worker should read the note as an ordinary "write a test asserting X" task. (The independence is the orchestrator's job — the base-directory read-scope + tool-fence — not something the note announces.)
