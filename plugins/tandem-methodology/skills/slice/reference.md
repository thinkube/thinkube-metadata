...
**Over-division is the same anti-pattern from the other side.** The layer/file split shatters one coherent capability into fragments; over-division does the same by reflex — the Agile habit of cutting work as small as it will go. This methodology rejects that reflex. For an AI worker, holding a larger coherent slice together is cheap, while every extra slice adds a merge, an acceptance gate, and a hand-off between them. **Coherence is cheap; coordination is what costs.** So when two candidate slices could share one statable "done," make them one — split only when the "done" itself splits (you cannot state a single green for the pair).
...
**A footprint is a list of files, not folders.** Spell out each file a unit may edit. A folder entry matches no file at the write guard — the check that a worker only writes the files it declared — so a folder in a footprint silently guards nothing and lets edits slip through unchecked. Always list the files themselves, never the directory that holds them.

Record each slice's work units — each `{ footprint, consumes?, execution, role?, note? }` — and **pass them to `create_slice` as `work_units` in step 6**. Classifying the shape in prose is not enough: **emitting the array is what instantiates the units** (omitting it leaves the slice with no work units to run). For a `fan-out` unit give each its `note` (the per-object task) so a worker is self-describing. The slice stays the validation envelope; work units are never independently verified.
