...
- **Contract-first when disjoint units share a contract.** Disjoint footprints are necessary but **not sufficient**: if the parallel units must agree on a shared **contract** (interface/type, name/key, schema, message, flag name), don't prose-pin it (prose-pinned conventions diverge — the opt-out-flag-name divergence when the contract-first gate was built). Make it a **contract-definition unit** authored first, and have each implementer **and** the test `depends_on` that **contract node** — never each other, never a slice-body convention. The implementers still fan out in parallel off the one node.
