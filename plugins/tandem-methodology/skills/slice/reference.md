...
## The role axis — who _grades_ the slice (Procedure step 3b, in full)
...
- **`role: "code"` (the default).** Authors the implementation. Every unit the footprint classifier produces is a code-author unit; omit `role` and it defaults to `"code"`. Nothing about the code path changes — the role axis is purely additive (SP-7 already ships the machinery that honors `role`, runs the assessor, and judges code-vs-test; this skill just emits the unit).
- **`role: "test"` (the held-out test-author).** **Exactly one per implementation slice.** It writes the slice's **acceptance probe** — the runnable check that grades the slice's "done" — and nothing else:
  - **Footprint = the repo's held-out acceptance-probe path**, declared in `.tandem/conventions.json` (`acceptanceProbe.sourcePath`) and filled with the spec id and **each AC ordinal the slice `satisfies`** (one probe per AC). It sits under SP-7's held-out `acceptance/` fence (`ACCEPTANCE_EVIDENCE_RE`), which **refuses a `role: "code"` author from writing there** — that fence is what makes the grade _independent_: the probe is authored by a worker that never wrote the implementation, so it can't quietly grade the code to its own shortcuts. Each path is exactly the `ac_verifications` command the auditor declared for that AC, so the written probe and the run command always match. **Tech-agnostic:** the repo supplies the language (this extension declares `src/acceptance/…test.ts` run by `node --test`; a Python repo would declare a `pytest` path).
  - **`consumes` the slice's contract file — never the code-authors' files.** The test-author needs the shared **contract** (the interface/type, name/key, schema, or wire message the code-authors also `consumes`) to write a probe against the agreed shape, but it must **not** read the implementation. So it hangs off the same **contract-definition unit** the implementers do (the contract-first pattern, now with a role) — not off any code unit. If the slice has no explicit contract unit, the test-author consumes whatever named artifact pins the observable surface it probes.
  - **Runs in parallel with the code-authors.** Its footprint (`acceptance/…`) is disjoint from every code footprint, so by the one footprint rule it parallelizes freely with them. Held-out **and** concurrent: the probe is being written while the code is being written, by a different hand, against the same contract.

**Why held-out.** A worker that writes both the code and its test grades itself — it tends to encode the same misreading into both, or to write the probe to pass the code it just wrote. Splitting the roles across a footprint fence (code can't touch `acceptance/`) forces the probe to commit to the _contract's_ meaning independently, so a green is evidence the implementation meets the agreed shape, not that one author agreed with themselves.

**Worked example.** In a Python repo whose `.tandem/conventions.json` declares `acceptanceProbe.sourcePath: "acceptance/sp{spec}_ac{ac}.py"` (run by `pytest`), a slice "Email/password login end-to-end" of SP-5 that `satisfies` AC 2, with a `POST /session` contract unit, fans out into `role: "code"` units for `models.py`, `routes.py` (each `consumes` the `POST /session` contract) plus **one `role: "test"` unit**: `{ footprint: "acceptance/sp5_ac2.py", consumes: "<the POST /session contract file>", execution: "serial", role: "test", note: "held-out probe for AC 2: form → POST /session → cookie set → redirect" }`. The footprint is the repo's recipe filled with `(spec=5, ac=2)` — exactly the `ac_verifications` command the auditor declared for AC 2. All three run concurrently — disjoint footprints, all hanging off the one contract node — and the test-author never reads `models.py` or `routes.py`. (A TypeScript repo would instead have declared `src/acceptance/SP-{spec}_AC-{ac}.test.ts` run by `node --test`; the methodology is identical, only the repo's recipe differs.)
