...
- **`role: "test"` (the held-out test-author).** **Exactly one per implementation slice.** It writes the slice's **acceptance probe** — the runnable check that grades the slice's "done" — and nothing else:
  - **Footprint `acceptance/SP-{n}_SL-{m}.*`.** Under SP-7's held-out `acceptance/` fence (`ACCEPTANCE_EVIDENCE_RE`), which **refuses a `role: "code"` author from writing there**. That fence is what makes the grade _independent_: the probe is authored by a worker that never wrote the implementation, so it can't quietly grade the code to its own shortcuts.
...
**Worked example.** A slice "Email/password login end-to-end" with a `POST /session` contract unit fans out into `role: "code"` units for `models.py`, `routes.py` (each `consumes` the `POST /session` contract) plus **one `role: "test"` unit**: `{ footprint: "acceptance/SP-3_SL-42.*", consumes: "<the POST /session contract file>", execution: "serial", role: "test", note: "held-out probe: form → POST /session → cookie set → redirect" }`. All three run concurrently — disjoint footprints, all hanging off the one contract node — and the test-author never reads `models.py` or `routes.py`.
