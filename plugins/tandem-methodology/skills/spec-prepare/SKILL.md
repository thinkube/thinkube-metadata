...
> **Decision-point protocol** (methodology `CLAUDE.md`): this is _human-paced_ authoring — converse → options → research → **read-back** → the human's explicit **"go."** Surface options as prose, never force convergence, and **approve ≠ execute**. Every choice you put to the user (including any `AskUserQuestion` fork) carries the situation that forces it, what each option means, and each option's pros and cons — never bare labels.
...
- A **verification map** (`ac_verifications`) — exactly one entry per AC, certified by the verifiability auditor — emitted before → Ready so the **closing** AI-verification gate has something to run. Each entry is either a **runnable probe** (`{ run, env }` — the auditor judges the AC verifiable and `write_spec` then auto-authors `run` from the repo's held-out acceptance-probe recipe, one probe per AC) or an **assessment** (`{ env: "assessment" }`) for a prose / UX / skill AC that no probe fits — graded by an independent assessor at the closing gate.
...
- **The TEP's RECEIPT, when one exists — read it FIRST.** A Spec minted from a thinking space has a sibling at `teps/TEP-{t}/receipt.json`: what the intent was built from. It carries the asks verbatim, the context digest read per ask (with citations, taken while the intent was being derived), the Why/Impact behind every item, whose words each item is (`direct` = the human's, `inferred` = the machine's), the evidence that closed each question, each unit's assessed complexity with its factor and grounds, and which item became which acceptance-criterion ordinal. Read it with `get_thinkube_file` before exploring the codebase — the digest is a reading of this repository taken for this intent, so it usually answers the architecture question you were about to search for.
  - **It is EVIDENCE, not authority.** The code wins on facts about the code; the ask wins on facts about intent. Where the receipt contradicts what you find, say so in the Design and cite both — never silently follow either.
  - **Never let it replace the interview.** It says what was decided and why; it does not say what the human wants now.
  - No receipt means the Spec was authored by hand. Proceed as before.
...
3. **Sizing check, then open with the interview and scaffold alongside.** Before you lay down the skeleton, sanity-check scope: one spec is one merge — the spec is the merge boundary. If this work would land as more than one independent merge (a different repo, a separate landing, or a different verification actor), it is more than one spec — say so and split before scaffolding; if not, keep it one. Then the conversation leads — ask the user for the spec's acceptance criteria as the first turn. Scaffold the file in parallel: `write_spec { thinking_space: <id>, spec: {n}, body }` with the exact shape of step 6 and placeholder bodies (`_(under discussion — see chat)_`). Mention the path once and, if the user wants a rendered view alongside the chat, point them at the Command Palette (_Markdown: Open Preview to the Side_) — optional, and never quote a keybinding (they don't fire reliably in browser / code-server).
...
- **Documentation impact**: as the ACs land, note which are **user-facing** — anything a reader can observe (a feature, CLI, API, config surface, install/upgrade step, or template behavior). That impact seeds each slice's `docs:` obligation in `/slice` (user-facing → `docs: required`; internal-only → `docs: n/a` + reason), so the → Done docs gate has something concrete to check. Record it in the Design (e.g. "the X page documents this") rather than as its own section.
...
7. **Audit each AC's verifiability and emit the verification map.** Once the ACs and Design are settled, run an **AC-verifiability auditor** — the **opening** AI-verification gate. Hand the drafted `## Acceptance Criteria` (plus the Design and the step-5 verification recipe) to an **adversarial pass via the `Task` tool** (the same delegation shape as `reviewer` / `verifier`), so each AC is interrogated with independent context. For each AC the auditor asks **five** questions — **actor** (the AI verifier, or the independent **assessor** for a prose AC), **environment** (`local`, `cluster`, or `assessment`), **availability before the gate it arms**, **controllability** (*walk through the probe — can it establish the AC's preconditions and drive the behaviour using only what the Design defines?* If the Design leaves a precondition's seam undefined — e.g. "with a secret **configured**" but never how one is configured, "when the feature is **enabled**" with no enablement surface — the probe author will invent that seam and the implementer will invent a different one, a guaranteed red. That is a **Design defect**: fix the Design to name the seam — a config env var, an injectable parameter, a setup call — then re-audit), and **lifecycle** — *is this criterion a one-time **transition** or a lasting **invariant**?* (When `teps/TEP-{t}/receipt.json` exists, hand the auditor the criterion's entry from it too: the Why/Impact that produced it, its evidence, and the ask it serves. Controllability is the question that most often fails for lack of context, and that is exactly the context — a criterion whose preconditions look undefined here is sometimes a criterion whose grounds were recorded upstream. It remains evidence, not authority: a receipt never rescues an AC the Design genuinely cannot drive.) A **transition** proves a change happened once (something removed / added / renamed): its truth is settled the instant the change ships, so it must **NOT** mint a runnable probe — that probe passes trivially forever and rots into a false positive nobody remembers (the mystery `grep "is X gone"` still running years later). Route a transition to **`assessment`**: the assessor confirms it **once** at the gate and leaves **nothing** in the permanent suite. An **invariant** is a behaviour that must always hold — only *that* earns a `verifiable` probe worth living in the suite forever. The auditor returns one of **three verdicts**: `verifiable`, `assessment`, or `needs-reframe(why)`.

   - **`verifiable`** — a runnable AC asserting an **invariant** (a behaviour that must always hold — worth a probe that lives in the suite forever). The auditor returns only the verdict + `env` (`local` or `cluster`); **`write_spec` then auto-authors the `run`** from the repo's held-out acceptance-probe recipe (`.tandem/conventions.json` → `acceptanceProbe.run`, filled with the spec id + AC ordinal), so each runnable AC is graded by its own **independently-authored per-AC probe**, never `npm test`. You do **not** hand-write the command. (A repo with no recipe falls back to its whole-suite command — self-graded — so declaring the recipe is what turns runnable independence on. `cluster` lifecycle commands the auditor names are left as-is.)
   - **`assessment`** — an AC that should leave **no permanent probe** behind. Two cases: (1) a **prose / UX / skill** AC that no probe fits (an instruction reads clearly, a methodology behaves as described); (2) a **one-time transition** — "verify `THINKUBE_APPROVAL_DIR` was removed", "the setting was added" — whose only runnable form would be a probe that passes trivially forever and rots. Emit `{ env: "assessment" }` — **no `run`, so no probe file is ever written**. The **closing** gate grades it via an **independent assessor** reading the artifact **once**; this is the home for every "verify X changed" criterion, never a `grep` that runs for the life of the repo. The auditor certifies this as a legitimate verdict.
...
- **Every AC carries a certified `ac_verifications` entry before Ready**, emitted only through `write_spec` (one serialization, both ends) — don't hand-craft the frontmatter or emit a partial map.
- **One spec per working repo by default — the spec is the merge boundary.** Fold the whole coherent change into a single spec unless a boundary forces a split (a different repo, a genuinely independent landing, or a different verification actor). Run this check before scaffolding. (`reference.md` → "Sizing a spec (one spec, one merge)".)
