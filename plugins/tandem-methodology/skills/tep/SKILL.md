...
- **Name implementing specs by what they deliver — never by an invented code/alias.** A TEP is authored _before_ its specs exist, so they have no thinking-space id yet, and an invented `SP-A`/`SP-B`-style alias is as opaque to a human as the base36 id the thinking space later mints. Throughout the Decision / Detailed Description / Implemented By, refer to each implementing spec by its **descriptive title** (the capability it delivers — e.g. _"Bounded, observable orchestration"_, not _"SP-B"_): the title is the one handle that is stable from TEP-authoring through spec creation and survives the thinking space minting a real `SP-{id}`. A real `SP-{id}` or PR is added later in `Implemented By` only as **landing traceability**, paired with the title — never as the way the TEP refers to the spec.
